...
function withDiagnostics(result) { return { ...result, diagnostics: parseDiagnostics(`${result.stdout}\n${result.stderr}`) }; }
...
export function restoreTarget(context, options = {}) { ensureProjectLike(context,'restore'); return shortCommand(context,['restore',context.targetInfo.path,'--nologo'],options,RestoreError,'dotnet restore'); }
...
export function cleanTarget(context, options = {}) { ensureProjectLike(context,'clean'); return shortCommand(context,['clean',context.targetInfo.path,'--nologo','-c',options.configuration ?? 'Debug'],options,ProcessExitError,'dotnet clean'); }
export function runTarget(context, options = {}) {
  ...
  if (context.targetInfo.kind === 'assembly') return spawnDotnet(context.dotnet,[context.targetInfo.path,...args],{cwd:options.cwd ?? context.targetInfo.directory,signal:options.signal,env:options.env});
