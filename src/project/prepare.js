...
async function inspectWorkloads(dotnet, required, cwd, signal) {
  if (!required?.length) return { required: [], installed: [], missing: [] };
  const result = await runDotnet(dotnet, ['workload', 'list'], { cwd, timeout: 60_000, signal });
  ...
  return { required: [...required], installed: [...installed], missing: required.filter(id => !installed.has(id)) };
}
...
async function inspectNativeAssets(targetInfo) {
  ...
  let data;
  try { data = JSON.parse(await fs.readFile(assetsPath, 'utf8')); }
  catch (error) {
  ...
  for (const target of Object.values(data.targets ?? {})) {
    for (const library of Object.values(target ?? {})) {
      ...
      for (const [assetPath, metadata] of Object.entries(library?.runtimeTargets ?? {})) if (metadata?.assetType === 'native') assets.add(assetPath);
    }
  }
...
async function ensureDotnetLegacy({ requirement, host, paths, options }) {
  let dotnet = await resolveDotnetHost({ requirement, paths, isolation: options.isolation ?? 'auto', dotnetPath: options.dotnetPath, dotnetArgsPrefix: options.dotnetArgsPrefix, env: options.env ?? process.env });
  let provisioned = false;
  if (!dotnet) {
    if ((options.isolation ?? 'auto') === 'system') throw new DotnetResolutionError('No compatible system .NET installation satisfies the target requirement.');
    dotnet = await provisionDotnet({ requirement, host, paths, env: options.env ?? process.env, offline: options.offline ?? false, artifactPath: options.artifactPath, expectedHash: options.expectedHash, fetchImpl: options.fetchImpl, signal: options.signal, onProgress: options.onProgress });
    provisioned = !dotnet.reused;
  }
  dotnet.info = dotnet.info ?? await verifyDotnet({ path: dotnet.path, argsPrefix: dotnet.argsPrefix, env: dotnet.env });
...
export async function prepareTarget(target, options = {}) {
  ...
  const host = services?.host?.detect ? services.host.detect(options.host) : detectHost(options.host);
  const paths = await ensurePathContext(await createPathContext({ mode: options.mode ?? 'shared', targetDir: targetInfo.directory, rid: host.rid, home: options.home }));
  const requirement = deriveRequirement(targetInfo, { sdk: options.sdk, runtime: options.runtime, defaultSdk: options.defaultSdk ?? '10.0', requireSdk: options.requireSdk ?? false });

  let dotnet = null;
  let provisioned = false;
  if (requirement.kind !== 'none') {
    const resolved = services?.environment?.ensure ? await services.environment.ensure({ requirement, host, paths, options }) : await ensureDotnetLegacy({ requirement, host, paths, options });
    dotnet = resolved.dotnet;
    provisioned = resolved.provisioned;
    if (dotnet && services?.execution && !dotnet.executor) dotnet.executor = services.execution;
  }

  let restoreResult = null;
  if (targetInfo.needsRestore && options.restore !== false) {
    restoreResult = await runDotnet(dotnet, ['restore', targetInfo.path, '--nologo'], { cwd: targetInfo.directory, timeout: options.restoreTimeout ?? 5 * 60_000, signal: options.signal });
    restoreResult.diagnostics = parseDiagnostics(`${restoreResult.stdout}\n${restoreResult.stderr}`);
    if (!restoreResult.ok) throw new RestoreError(`dotnet restore failed for ${targetInfo.path}.`, { details: { result: restoreResult } });
  }

  const workloads = dotnet ? await inspectWorkloads(dotnet, targetInfo.workloads, targetInfo.directory, options.signal) : { required: [], installed: [], missing: [] };
  ...
  const state = {
    ...
    execution,
    services: services ? { execution: services.execution?.constructor?.name ?? null, environment: services.environment?.constructor?.name ?? null, project: services.project?.constructor?.name ?? null } : null,
    preparedAt: new Date().toISOString()
  };

  let statePath = null;
  const shouldWriteState = options.writeState === true || (options.writeState !== false && paths.mode !== 'temporary');
  if (shouldWriteState) statePath = await writeState(targetInfo, paths, state);
  return { targetInfo, host, paths, requirement, dotnet, provisioned, restoreResult, workloads, nativeAssets, state, statePath, services, ready: workloads.missing.length === 0, readinessWarnings };
}
