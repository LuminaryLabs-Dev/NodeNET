...
test('service registry allows an explicit replacement provider', async () => {
  ...
  registry.register(definePlugin({ name: 'test.execution.one', provides: [SERVICE.EXECUTION], register(_registry, { provide }) { provide(SERVICE.EXECUTION, { id: 'one' }); } }));
  registry.register(definePlugin({ name: 'test.execution.two', provides: [SERVICE.EXECUTION], replace: true, register(_registry, { provide }) { provide(SERVICE.EXECUTION, { id: 'two' }); } }));
